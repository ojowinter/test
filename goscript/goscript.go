...
type transform struct {
	fset *token.FileSet
	src  *value        // Go sintaxis to translating
	dst  *bytes.Buffer // sintaxis translated to JS
...
func Compile(filename string) error {
	...
	*/
	...
	trans.dst.WriteString("/* Generated by GoScript <github.com/kless/GoScript> */")
	...
	if len(trans.public) != 0 {
		for i, v := range trans.public {
			if i == 0 {
				trans.dst.WriteString(NL + NL + "export " + v)
			} else {
				trans.dst.WriteString("," + SP + v)
			}
		}

		trans.dst.WriteString(";" + NL)
	}

	// === Write
	fmt.Print(trans.dst.String()) // TODO: delete*/

	//jsFile := strings.Replace(filename, path.Ext(filename), ".js", 1)
	//return ioutil.WriteFile(jsFile, trans.dst.Bytes(), 0664)
	return nil
}
